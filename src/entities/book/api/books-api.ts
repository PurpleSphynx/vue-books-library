...
export function getBooks(q: BookQuery = {}) {
  return request<{ items: Book[]; pagination: Pagination }>(`/books${query(q)}`)
}
...
export function createBook(input: BookInput, cover: File) {
  return request<Book>('/books', { method: 'POST', body: form(input, cover) })
}

export function updateBook(id: number, input: BookInput, cover?: File) {
  return cover
